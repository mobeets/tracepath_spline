...
def myrandom(size):
    ...
    res = 1.-2.*np.random.random(size=size)
...
class Render(object):
    ...
    def sand_paint(self,left,right):
        ...
        left_tck,left_u = interpolate.splprep([left[:,0], left[:,1]],s=0)

        steps = np.linspace(0,1,num_points)
        left_out = interpolate.splev(steps,left_tck)
        left_res = np.column_stack(left_out)

        right_tck,right_u = interpolate.splprep([right[:,0], right[:,1]],s=0)

        steps = np.linspace(0,1,num_points)
        right_out = interpolate.splev(steps,right_tck)
        right_res = np.column_stack(right_out)

        dxx = right_res[:,0]-left_res[:,0]
        dyy = right_res[:,1]-left_res[:,1]
        dd = np.square(dxx)+np.square(dyy)
        np.sqrt(dd,dd)
        aa = np.arctan2(dyy,dxx)

        #r,g,b = self.colors[ int(np.random.random()*self.n_colors) ]
        ...
        for i,((lx,ly),(rx,ry)) in enumerate(zip(left_res,right_res)):
            ...
            xp = lx + scales*np.cos(a)
            yp = ly + scales*np.sin(a)
...
class Path(object):
    ...
    def noise(self):
        ...
        alpha_noise = myrandom(len(self.xy_new))*np.pi
        noise = np.column_stack([np.cos(alpha_noise), np.sin(alpha_noise)])*self.r*0.1
        self.xy_new += noise
...
def main():
    ...
    for i in count():
        ...
        draw_start,draw_stop = get_limit_indices(xy,top=START_Y,bottom=STOP_Y)
        draw_circles(xy[draw_start:draw_stop,:], np.ones(draw_stop-draw_start)*ONE)
